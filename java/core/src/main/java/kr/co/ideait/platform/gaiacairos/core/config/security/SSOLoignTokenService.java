...
public class SSOLoignTokenService {
	...
	private static final byte[] ssoKeyBytes;
	...
	static {
		try {
			...
			String jwtKeyPlatform = "gaia.secret.jwtKeyPlatform";
			String propSsokey = "gaia.secret.ssokey";

			secretKey = readSecretKeyFromFile(new File(prop.getProperty(jwtKeyPlatform, System.getProperty(jwtKeyPlatform))));
			String ssokey = prop.getProperty(propSsokey, System.getProperty(propSsokey));
...
//		return request.getHeader(KeyConstants.AUTH_KEY_OLD);
	...
	private static SecretKey readSecretKeyFromFile(File secKeyFile) throws IOException {
