...
//    }
     ...
     * iCube 문서 생성
     *
     * @param requestParams 생성하고자 하는 문서의 파라미터
     * @param files 파일 리스트
     * @param headers 헤더(토큰 등 포함)
     * @return 문서정보(DcStorageMain) List / null
     */
...
//            String navigationListStr = objectMapper.writeValueAsString(response.getDetails().get("navigationList"));
    ...
    public Map<String, Object> createApprovalDocument(Map<String, Object> requestParams){
        Map<String, Object> result = new HashMap<>();

        try {
            Map<String, String> headers = new HashMap<>();
            String url = String.format("%s/%s", domain, documentServiceProp.getApi().getCreateApprovalDocument());
            Result response = restClientUtil.sendPost(url
                    , getHeaderMap(headers)
                    , requestParams
                    , Result.class
            ...
            if(response == null){
                throw new GaiaBizException(ErrorType.ETC,"[createApprovalDocument] 문서관리 통신 실패");
            }

            if(response.isOk()){
                return response.getDetails();
            }else{
                throw new GaiaBizException(ErrorType.ETC, "[createApprovalDocument] 문서관리에 결재 문서 생성 실패.");
            }
        } catch (GaiaBizException e) {
            log.error("[createApprovalDocument()]:{}", e.getMessage());
            result.put("resultCode", "01");
