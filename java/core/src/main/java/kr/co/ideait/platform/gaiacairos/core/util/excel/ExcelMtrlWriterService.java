...
public class ExcelMtrlWriterService {
    ...
    // * @throws Exception
    // */
    // public void writeExcel(HttpServletResponse response, List<RawCostItem>
    // rawList, String sheetNm, String cntrctNm) throws Exception {
    // CostItemNode root = buildCostTree(rawList); // 트리 구조 생성

    // // 템플릿 엑셀 파일 불러오기
    // ClassPathResource template = new
    // ClassPathResource("public/excel/calculator_sample2.xlsx");
    ...
    // // 트리 구조 순회하면서 데이터 작성
    // writeRows(sheet, root.getChildren(), rowNum, centerStyle, rightAlignStyle,
    // excelRows);
    ...
    // String encodedDownloadFile = URLEncoder.encode(fileName,
    // "UTF-8").replace("+", "%20");
    ...
    // // 재귀적으로 행 작성
    // private void writeRows(Sheet sheet, List<CostItemNode> nodes, AtomicInteger
    // rowNum, CellStyle style, CellStyle rightAlignStyle, List<ExcelRow> excelRows)
    // {
    ...
    // if (level == 1) {
    // writeRows(sheet, node.getChildren(), rowNum, style, rightAlignStyle,
    // excelRows);
    // continue;
    // }
    ...
    // String gubun = "소계".equals(item.getCstCalcItNm()) ? getSubtotalCode() :
    // item.getCstCalcItCd();
    ...
    // private void applyVerticalMerge(Sheet sheet, List<ExcelRow> rows, int
    // colIndex) {
    // String prev = null;
    // int start = -1;
    ...
    // if (!parent.getChildren().isEmpty() && parent.getData() != null &&
    // parent.getData().getAMenuLevel() == 2) {

    // long total =
    // parent.getChildren().stream().mapToLong(CostItemNode::getTotalCostAmount).sum();
    ...
    // current.getData().getAMenuLevel() != level) {
    // current = current.getParent();
    // }
    // return (current != null && current.getData() != null) ?
    // current.getData().getCstCalcItNm() : "";
    // }
