...
public class MainmtrlReqfrmComponent extends AbstractComponent {
     ...
     */
    @Transactional
    public void inspectionRequestList(List<Map<String, Object>> paramList, HttpServletRequest request, String usrId, String usrNm, String pjtNo) {
        for (Map<String, Object> param : paramList) {
            ...
            // 이미 검수자가 지정되어 있으면 skip
            if (mainmtrlReqfrm.getCmId() != null && !mainmtrlReqfrm.getCmId().isBlank()) {
                log.info("이미 검수요청 보낸 항목은 건너뜀: {}", mainmtrlReqfrm);
                continue;
            }
     ...
     * 검수요청 - 메일 전송
     */
    public void inspectionRequest(Map<String, Object> param, HttpServletRequest request, String usrId, String usrNm, String pjtNo) {
        ...
        String linkUrl = String.format(
                ...
                baseUrl, UserAuth.get(true).getPjtNo(), cntrctNo, reqfrmNo);
        ...
        insertParam.put("reqAppDiv", "QUA"); // 업무구분: 품질검수
    ...
    public void addMtrlReqfrmResult(MainmtrlReqfrmForm.MainmtrlReqfrm mainmtrlReqfrm, String usrId) {
        ...
        if ("02".equals(mainmtrlReqfrm.getRsltCd())) {
            ...
            mainmtrlReqfrm.getPartialFailList().forEach((partialFail) -> {
                ...
                Integer passQty = partialFail.getPassQty();
                Integer failQty = partialFail.getFailQty();

                MybatisInput input = new MybatisInput().add("reqfrmNo", reqfrm).add("cntrctNo", cntrctNo)
                        .add("gnrlexpnsCd", gnrlexpnsCd).add("passQty", passQty).add("failQty", failQty);
            ...
            MybatisInput input = new MybatisInput().add("reqfrmNo", reqfrm).add("cntrctNo", cntrctNo)
                    .add("gnrlexpnsCd", gnrlexpnsCd).add("passYn", passYn);
            mainmtrlReqfrmService.updateMainmtrlList(input);
        }
    }
     ...
     */
    @Transactional
    public void paymentRequest(List<Map<String, String>> paramList, String isApiYn, String pjtDiv) {
