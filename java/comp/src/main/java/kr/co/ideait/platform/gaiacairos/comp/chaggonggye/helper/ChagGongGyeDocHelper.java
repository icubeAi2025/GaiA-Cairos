    ...
    public Map<String, Object> makeChagGongGyeDoc(Map<String, Object> addData, String naviId, String docId) {
        ...
        try {
            ...
            if(addParamsData.size() > 0) {
                addParamsData.forEach(item -> {
                    ...
                    if("TXTA".equals(item.get("attrbt_type"))) {
                        ...
                        fileParamNm.append("@"+item.get("attrbt_cd"));
                        ...
                        try {
                            multipartBodyBuilder
                                    ...
                                    .header("Content-Disposition", String.format("form-data; name=%s; filename=%s",(String) item.get("attrbt_cd"), URLEncoder.encode(addFile.getFilename(), StandardCharsets.UTF_8)));
                        } catch (IOException e) {
            ...
            multipartBodyBuilder
                    ...
                    .header("Content-Disposition", String.format("form-data; name=%s; filename=%s",docParam[1]+docParam[2], URLEncoder.encode(resource.getFilename(), StandardCharsets.UTF_8)));
                ...
                if (checkValue) {
                    ...
                    if (defaultParamsData.get("file_nm") != null || !"".equals(defaultParamsData.get("file_nm")) || defaultParamsData.get("file_nm") != null) {
                        ...
                        multipartBodyBuilder
                                ...
                                .header("Content-Disposition", String.format("form-data; name=%s; filename=%s","orgchrt_atch_file", URLEncoder.encode(img.getFilename(), StandardCharsets.UTF_8)));
                    }
                }
            }
    ...
    public Map<String, Object> mergeChagGongGyeDoc(List<MultipartFile> chagGonGyeDoc, List<Map<String, Object>> mergeAttachments, String naviId, String docId) {
        ...
        try {
            ...
            for (MultipartFile doc : chagGonGyeDoc) {
                if (!doc.isEmpty()) {
                    multipartBodyBuilder
                            ...
                            .header("Content-Disposition", String.format("form-data; name=%s; filename=%s", "file", URLEncoder.encode(doc.getOriginalFilename(), StandardCharsets.UTF_8)));
                }
            }
            ...
            for (Map<String, Object> item : mergeAttachments) {
                ...
                if (!mergeResource.exists() || !mergeResource.isFile()) {
                ...
                multipartBodyBuilder
                        ...
                        .header("Content-Disposition", String.format("form-data; name=%s; filename=%s", "file", URLEncoder.encode(mergeResource.getFilename(), StandardCharsets.UTF_8)));
            }
