...
public class ApprovalComponent extends AbstractComponent {
				...
				if(!EapprovalHelper.APPROVAL_DOC.equals(findApDoc.getApType())) {
					switch(findApDoc.getApType()) {
						...
						case EapprovalHelper.MAINMTRL_REQFRM_DOC:
							...
							break;
						default:
							throw new GaiaBizException(ErrorType.NOT_FOUND, String.format("updateApDocStatus >>> 존재하지 않는 문서 타입입니다: %s", findApDoc.getApType()));
					}
				}
			}
		});
	...
	public Map<String, Object> selectApTypeResources(String apDocId, String apType) {
		switch(apType) {
			...
			// 주요자재 검수요청서
			case EapprovalHelper.MAINMTRL_REQFRM_DOC:
				return mainmtrlReqfrmService.selectMainmtrlReqfrmByApDocId(apDocId);
			default:
				throw new GaiaBizException(ErrorType.NOT_FOUND, String.format("selectApTypeResources >>> 존재하지 않는 문서 타입입니다: %s", apType));
		}
	}
	...
	public Map<String, Object> receiveApiOfApprovalInsert(String msgId, Map<String, Object> params) {
		...
		try {
			if("GACA8200".equals(msgId)) {
				log.info("receiveApiOfApprovalInsert >>> API 연동 params : {}", params);
...
//						break;
					...
					// 주요자재 검수요청서
					case EapprovalHelper.MAINMTRL_REQFRM_DOC:
						mainmtrlReqfrmService.insertResourcesToApi(
								objectMapper.convertValue(params.get("report"), CwMainmtrlReqfrm.class),
								objectMapper.convertValue(resources.get("mainmtrls"), new TypeReference<List<CwMainmtrl>>() {}),
								objectMapper.convertValue(resources.get("photo"), new TypeReference<List<CwMainmtrlReqfrmPhoto>>() {}),
								objectMapper.convertValue(resources.get("mainmtrlReqfrmFileInfo"), new TypeReference<List<Map<String, Object>>>() {})
						);
					break;
					default:
						throw new GaiaBizException(ErrorType.NOT_FOUND, String.format("receiveApiOfApprovalInsert >>> 존재하지 않는 report 타입입니다: %s", reportType));
				}
