...
public class QualityinspectionComponent extends AbstractComponent {
    ...
    public void inspectionRequestList(CommonReqVo commonReqVo, List<Map<String, Object>> paramList, HttpServletRequest request) {
        for (Map<String, Object> param : paramList) {
            ...
            qualityService.deleteReqItem(qltyIspId);
                


            quality.setRsltDocNo(null);
            quality.setRsltDt(null);
            quality.setRsltCd(null);
            quality.setApprvlDt(null);
            quality.setApprvlId(null);
            quality.setApprvlStats(null);

            qualityService.createQuality(quality);

            // 전자결재 존재 시 관련 전자결재 문서도 삭제관련 전자결재 문서도 삭제
            if(quality.getApDocId() != null && !quality.getApDocId().isBlank()) {
                approvalRequestService.deleteApDocByApDocId(quality.getApDocId());
            }
            
            
            inspectionRequest(commonReqVo, param, request, quality);
        }
    }

    public void inspectionRequest(CommonReqVo commonReqVo, Map<String, Object> param, HttpServletRequest request, CwQualityInspection quality) {
