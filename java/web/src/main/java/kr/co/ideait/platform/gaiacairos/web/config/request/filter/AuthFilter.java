...
public class AuthFilter extends OncePerRequestFilter {
    ...
    @Value("${gaia.session-expired:32400}") //60 * 60 * 9
...
//                            loginSuccessHandler.successHandle(user);
                            ...
                            if (userInfo == null) {
                                userInfo = authComponent.loginUserInfo(user.getUsrId());
                                session.setAttribute("userInfo", userInfo);
                            }
                            threadContextHelper.set("userInfo", userInfo);
                        ...
                        if ("TOKEN_EXPIRED".equals(accTokenValidDto.getErrorName())) {
                            ...
                            if(refreshToken != null) {
                                ...
                                if (refreshTokenValidDto.isValid()) {
                                    ...
                                    // 토큰을 쿠키에 저장
                                    cookieService.setHttpOnlyCookie(response, cookieVO.getTokenCookieName(), reAccessToken, expired); //8시간
...
//        log.info("accessUri: {} ", accessUri);
        log.info("url match: {}", PatternMatchUtils.simpleMatch(checkList, uri));
