...
public class AuthPageController extends AbstractController {
	 ...
	 */
	private Map<String, Object> executeLogin(SmUserInfo selectUser, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<User.AccessAuthority> accessAuthorizes = Arrays.asList(authories);

			// 1. 포탈 사용자 정보로 토큰을 생성한다.
			String accessToken 	= tokenService.generate(selectUser.getUsrId(), selectUser.getLoginId(), accessAuthorizes); //30분
...
//			response.addCookie(cookie);
	 ...
	 * 로컬&개발&스테이징 로그인 처리용
	 */
	private Result doLogin(Map<String, String> param, HttpServletRequest request, HttpServletResponse response) {
     ...
     * 운영 공통 로그인처리
     */
	private String doSSOLogin(String redirectPath, HttpServletRequest request, HttpServletResponse response) {
		String usrId 	= "";
		String ssoToken = "";
		String redirectUrl = "";
		...
		final String strPort = port != 80 ? ":" + port : "";

		if (PlatformType.PGAIA.getName().equals(platform)) {
			redirectUrl = String.format( "redirect:%s://%s%s%s", scheme, domain, strPort, StringUtils.isEmpty(contextPath) ? "" : "/" + contextPath );
		} else {
			redirectUrl = String.format( "redirect:%s://%s%s%s", scheme, domain, strPort, StringUtils.isEmpty(contextPath) ? "" : "/" + contextPath );
		}
     ...
     */
	@GetMapping("/login")
	public String loginPage(@RequestParam(value = "redirectPath", defaultValue = "") String redirectPath, @RequestParam(value = "isBackdoor", defaultValue = "false") boolean isBackdoor, HttpServletResponse response, HttpServletRequest request) {
	 ...
	 */
	@PostMapping("/login")
	public String login(@RequestParam Map<String, String> param, HttpServletResponse response, HttpServletRequest request, ModelMap modelMap) {
	...
	public String newSsoLogin(@RequestParam(value = "redirectPath", defaultValue = "") String redirectPath, HttpServletResponse response, HttpServletRequest request) {
		return doSSOLogin(redirectPath, request, response);
	}
	 ...
	 */
	@GetMapping("/ncpLogin")
	public String ncpSsoLogin(@RequestParam(value = "redirectPath", defaultValue = "") String redirectPath, HttpServletResponse response, HttpServletRequest request) {
		return doSSOLogin(redirectPath, request, response);
	}
