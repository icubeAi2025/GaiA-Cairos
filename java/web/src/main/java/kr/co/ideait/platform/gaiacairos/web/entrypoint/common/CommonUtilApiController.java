...
public class CommonUtilApiController extends AbstractController {
	...
	public Result fileCheck(CommonReqVo commonReqVo, @RequestBody FileService.FileMeta fileMeta) {
		Resource resource = fileService.getFile(fileMeta.getFilePath());
		if(resource == null){
			return Result.nok(ErrorType.NO_DATA,"존재하지 않는 파일입니다.");
		}
		else{
			return Result.ok();
		}
	...
	public ResponseEntity<Resource> getPreviewData(CommonReqVo commonReqVo, @RequestParam("filePath") String filePath) {
		if(filePath != null && !filePath.isEmpty()){
			HashMap<String,Object> result = commonUtilComponent.getPreviewData(filePath);
			...
			if(result != null) {
				...
				if("pdf".equals(ext)){
					return ResponseEntity.ok()
							.contentType(MediaType.APPLICATION_PDF)
							.header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + "temp" + "\"")
							.header(HttpHeaders.CONTENT_ENCODING, "binary")
							.header(HttpHeaders.PRAGMA, "no-cache")
							.header(HttpHeaders.EXPIRES, "-1")
							.body(resource);
