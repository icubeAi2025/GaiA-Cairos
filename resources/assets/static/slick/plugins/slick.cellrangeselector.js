  ...
  function CellRangeSelector(options) {
    ...
    function handleDragInit(e, dd) {
      ...
      if (_gridOptions.frozenRow > -1 && _isBottomCanvas) {
        //pyk _rowOffset = ( _gridOptions.frozenBottom ) ? $('.grid-canvas-bottom').height() : $('.grid-canvas-top').height();
        _rowOffset = ( _gridOptions.frozenBottom ) ? $('.grid-canvas-bottom', _grid.getContainerNode()).height() : $('.grid-canvas-top', _grid.getContainerNode()).height();
      }
      ...
      if (_gridOptions.frozenColumn > -1 && _isRightCanvas) {
        ...
        _columnOffset = $('.grid-canvas-left', _grid.getContainerNode()).width();
      }
    ...
    function handleDrag(e, dd) {
      ...
      if(_isRightCanvas){
      	if(end.cell <= _gridOptions.frozenColumn){
					endCell = _gridOptions.frozenColumn + 1;
      	}else{
      		endCell = end.cell;
      	}   	
      }else{
      	if(_gridOptions.frozenColumn < 0 || _gridOptions.frozenColumn == null){
      		endCell = end.cell;
      	}else if(end.cell > _gridOptions.frozenColumn){
      		endCell = _gridOptions.frozenColumn;
      	}else{
      		endCell = end.cell;
      	}
      }
...
//        return;
//      }
      dd.range.end = {row: end.row, cell: endCell};
