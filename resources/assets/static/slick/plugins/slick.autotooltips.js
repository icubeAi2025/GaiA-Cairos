// pyk 2022/03/03 Handlebars 빼버릴려고
  ...
  function AutoTooltips(options) {
    ...
    function handleMouseEnter(e) {
      ...
      if (cell) {
        ...
        // pyk 2022/03/03 var text = options.useToolTipForCells? useTooltipOfColumn(cell) : useContentOfCell($node);
        var text = useContentOfCell($node);
    ...
    function handleHeaderMouseEnter(e, args) {
      ...
      if(! column)return;//pyk
      if (!column.toolTip) {
        // pyk $node.attr("title", ($node.innerWidth() < $node[0].scrollWidth) ? column.name : "");
        $node.attr("title", column.name);
      }
    }
