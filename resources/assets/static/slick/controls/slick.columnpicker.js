(function ($) {
  ...
  function SlickColumnPicker(columns, grid, options) {
    ...
    function createColumnsList(_columns, firstLevel) {
      ...
      //pyk _columns.forEach(function (column) {
    ...
    function group(columns, agregators) {
      var grouping =
        columns.map(function (column) {
          ...
          if (agregators)
            //pyk agregators.forEach(function (agregator) {
          	_.each(agregators, function(agregator){
    ...
    function calculateTotal(column, items) {
      ...
      switch (column.totalizationType) {
        case Slick.Data.Aggregators.Sum:
          ...
          column.total = +Big(column.total).plus(totals.sum[field]||0);
          break;
        case Slick.Data.Aggregators.Avg:
          // column.total += totals.avg[field];
          column.total = +Big(column.total).plus(totals.avg[field]||0);
          break;
      }
    }
