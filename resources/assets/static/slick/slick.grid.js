...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    var $container;
    var uid = "slickgrid_" + crypto.getRandomValues(new Uint32Array(1))[0];
    var self = this;
    ...
    var prevActiveRow, prevActiveItemId, activeItemId;// pyk 2022 06 16
    ...
    var _columnDefinitions;// 2022 07 19 pyk
    //////////////////////////////////////////////////////////////////////////////////////////////
    // Initialization
    function _setTreeColumns(colDefs){
      _columnDefinitions = _.cloneDeep(colDefs);// 2022 07 19 pyk
      treeColumns = new Slick.TreeColumns(colDefs);
      columns = treeColumns.extractColumns();
    };

    function _updateColumnsById(){
      columnsById = {};
    ...
    function init() {
      ...
      $container
          .empty()
          .css("overflow", "hidden")
          //openwax .css("outline", 0)
          .addClass(uid)
          //pyk .addClass("ui-widget")
          ;
      ...
      var cet = isIE ? '' : 'contenteditable="true"';
      // pyk 2022 03 30 remove hidefocus
      $focusSink = $("<div "+cet+" class='slick-focus' tabIndex='0' style='position:fixed;width:0;height:0;top:0;left:0;'></div>").appendTo($container);//openwax outline:0;
      ...
      if(options.hideHeader){
        $paneHeaderL.toggle(false);
        $paneHeaderR.toggle(false);
      }
      ...
      $headerScrollerL = $("<div class='x-state-default slick-header slick-header-left' />").appendTo($paneHeaderL);
      $headerScrollerR = $("<div class='x-state-default slick-header slick-header-right' />").appendTo($paneHeaderR);
      ...
      $headerRowScrollerL = $("<div class='x-state-default slick-headerrow' />").appendTo($paneTopL);
      $headerRowScrollerR = $("<div class='x-state-default slick-headerrow' />").appendTo($paneTopR);
      ...
      $topPanelScrollerL = $("<div class='x-state-default slick-top-panel-scroller' />").appendTo($paneTopL);
      $topPanelScrollerR = $("<div class='x-state-default slick-top-panel-scroller' />").appendTo($paneTopR);
      ...
      // pyk 2022 03 30 remove hidefocus
      $viewportTopL = $("<div class='slick-viewport slick-viewport-top slick-viewport-left' tabIndex='0' />").appendTo($paneTopL);
      $viewportTopR = $("<div class='slick-viewport slick-viewport-top slick-viewport-right' tabIndex='0' />").appendTo($paneTopR);
      $viewportBottomL = $("<div class='slick-viewport slick-viewport-bottom slick-viewport-left' tabIndex='0' />").appendTo($paneBottomL);
      $viewportBottomR = $("<div class='slick-viewport slick-viewport-bottom slick-viewport-right' tabIndex='0' />").appendTo($paneBottomR);
      ...
      // pyk 2022 03 30 remove hideFocus
      $canvasTopL = $("<div class='grid-canvas grid-canvas-top grid-canvas-left' tabIndex='0' />").appendTo($viewportTopL);
      $canvasTopR = $("<div class='grid-canvas grid-canvas-top grid-canvas-right' tabIndex='0' />").appendTo($viewportTopR);
      $canvasBottomL = $("<div class='grid-canvas grid-canvas-bottom grid-canvas-left' tabIndex='0' />").appendTo($viewportBottomL);
      $canvasBottomR = $("<div class='grid-canvas grid-canvas-bottom grid-canvas-right' tabIndex='0' />").appendTo($viewportBottomR);
      ...
      // footer Row
      $footerRowScrollerR = $("<div class='x-state-default slick-footerrow' />").appendTo($paneTopR);
      $footerRowScrollerL = $("<div class='x-state-default slick-footerrow' />").appendTo($paneTopL);
    ...
    function getHeadersWidthL() {
      ...
      //pyk columns.forEach(function(column, i) {
      var column;
    ...
    function disableSelection($target) {
      if ($target && $target.jquery) {
        $target
            // pyk 2022 03 30 .attr("unselectable", "on")
    ...
    function createColumnFooter() {
      ...
      for (var i = 0; i < columns.length; i++) {
        ...
        var footerRowCell = $("<div class='x-state-default slick-footerrow-column l" + i + " r" + i + "'></div>")
          .data("column", m)
    ...
    function createColumnGroupHeaders() {
      ...
      for (var index = 0; index < $groupHeadersL.length; index++) {
        ...
        for (var indexGroup in groupColumns) {
          ...
          $("<div class='x-state-default slick-group-header-column' />")
            .html("<span class='slick-column-name'>" + m.name + "</span>")
            // pyk 2022 03 30
            .attr("id", "" + uid + (m.id||('_'+_.uniqueId())))
    ...
    function createColumnHeaders() {
      function onMouseEnter() {
        //pyk $(this).addClass("ui-state-hover");
      }

      function onMouseLeave() {
        //pyk $(this).removeClass("ui-state-hover");
      }
      ...
      for (var i = 0; i < columns.length; i++) {
        ...
        var header = $("<div class='x-state-default slick-header-column' />")
          .html("<span class='slick-column-name'>" + m.name + "</span>")
          //pyk for firefox on bootstrap .width(m.width - headerColumnWidthDiff)
          .attr("id", "" + uid + m.id)
          .attr("title", m.toolTip || "")
          .data("column", m)
          .addClass(m.headerCssClass || "")
          ...
          .outerWidth(m.width)//.width(m.width - headerColumnWidthDiff)
          ;
        ...
        //pyk if (options.showHeaderRow) {
          var headerRowCell = $("<div class='x-state-default slick-headerrow-column l" + i + " r" + i + "'></div>")
              .data("column", m)
    ...
    /*컬럼 클릭 시, 정렬 되는 부분(asc, desc) */
    function setupColumnSort() {
      $headers.click(function (e) {
        //pyk 2018-04-23
        if($(e.target).is('input.slick_allcheck') || $(e.target).is('.slick-resizable-handle')){
          return;
        }
...
//        });
      ...
      var temp = treeColumns.getColumnsInDepth($groupHeadersL.length - 1);
      var groupColumn, column;
      for(var i = 0, len = temp.length; i < len; i++){
      	groupColumn = temp[i];
        startLimit = endLimit;
        endLimit += groupColumn.columns.length;
        for(var i2 = 0, len2 = groupColumn.columns.length; i2 < len2; i2++){
        	column = groupColumn.columns[i2];
          if (column.id === idColumn)
            groupColumnOfPreviousPosition = groupColumn;
          if(groupColumnOfPreviousPosition)break;
        }
        if(groupColumnOfPreviousPosition)break;
      }
      endLimit--;
    ...
    function setupColumnReorder() {
      ...
      $headers.sortable({
        ...
        helper: "clone",
        placeholder: "slick-sortable-placeholder x-state-default slick-header-column",
        start: function (e, ui) {
          ui.placeholder.outerWidth(ui.helper.outerWidth());//ui.placeholder.width(ui.helper.outerWidth() - headerColumnWidthDiff);
        ...
        stop: function (e, ui) {
          ...
          _updateColumnsOnly(reorderedColumns);
...
//              updateCanvasWidth()
    ...
    function measureCellPaddingAndBorder() {
      ...
      el = $("<div class='x-state-default slick-header-column' style='visibility:hidden'>-</div>").appendTo($headers);
      headerColumnWidthDiff = headerColumnHeightDiff = 0;
      style = getComputedStyle(el[0]);
      ...
      for (var depth = $groupHeadersL.length - 1; depth >= 0; depth--) {
        ...
        $().add($groupHeadersL[depth]).add($groupHeadersR[depth]).each(function(i) {
          ...
          $groupHeader.children().each(function() {
            ...
            // pyk m.columns.forEach(function() {
            var column;
            for(var i = 0, len = m.columns.length; i < len; i++){
            	column = m.columns[i];
              var $headerColumn = $groupHeader.next().children(':eq(' + (currentColumnIndex++) + ')');
              m.width += $headerColumn.outerWidth();
            }

            $groupHeaderColumn.outerWidth(m.width);//$groupHeaderColumn.width(m.width - headerColumnWidthDiff);
    ...
    function applyColumnHeaderWidths() {
      ...
      for (var i = 0, headers = $headers.children(), ii = headers.length; i < ii; i++) {
        ...
        if (h.outerWidth() !== colWidth) {
          h.outerWidth(colWidth);
        }
      }
    ...
    function setColumns(treeColumns) {
      _setTreeColumns(treeColumns);
      _updateColumnsById();
      _refreshColumns();
    }

    function _updateColumnsOnly(reorderedColumns) {
    ...
    function setOptions(args) {
      ...
      // 2022 07 19 pyk setColumns(treeColumns.extractColumns());
      // pyk 2022 컬럼 폭이 재조정 되버려서 처리하자
      if(_columnDefinitions){
        _.each(_columnDefinitions, function(col){
          if(! (col && col.id))return;
          var col2 = self.getColumnById(col.id);
          if(! col2)return;
          if(col.width != col2.width)col.width = col2.width;
        });
      }
      setColumns(_columnDefinitions);
      render();
    }
    ...
    function setHeaderRowVisibility(visible) {
      if (options.showHeaderRow != visible) {
        ...
        if (visible) {
          ...
          // pyk 20160405 안보인 상태에서 스크롤 한다음에 보이게 하면 엇갈려서 아래 추가
          $headerRowScrollContainer[0].scrollLeft = $viewportScrollContainerX[0].scrollLeft;// pyk
        } else {
    ...
    function appendRowHtml(stringArrayL, stringArrayR, row, range, dataLength) {
      ...
      var rowHtml = "<div class='x-widget-content " + rowCss + "' style='top:"
    ...
    function appendCellHtml(stringArray, row, cell, colspan, item) {
      ...
      // pyk
      var value = item ? getDataItemValueForColumn(item, m) : null;
      var cellStyleFunction = m.cellStyleFunction || options.cellStyleFunction;
      if(cellStyleFunction && item){
    	  var cc = cellStyleFunction(row, cell, value, m, item, self);
    	  if(cc){
    		  cellCss += (" "+cc);
    	  }
      }
    ...
    function resizeCanvas() {
      ...
      $paneTopL.css({
        // pyk 2022 05 18
        'top': options.hideHeader ? 0 : $paneHeaderL.height(), 'height': paneTopH
      });
...
viewportHasHScroll = (canvasWidth > viewportW - scrollbarDimensions.width);// pyk 20190919 컬럼폭 줄이고 나서 윈도 리사이즈 하면 맨아래 로우 스크롤 안되는 문제
      ...
      if (!handled) {
        if (!e.shiftKey && !e.altKey && !e.ctrlKey) {
          if (e.which == 27) {
            ...
            // pyk 2022/03/18 황요청
            if(! getEditorLock().isActive()){
              navigate(options.navigateWhenEnter || 'down');
            }
            handled = true;
          }
    ...
    function handleClick(e) {
      ...
      if ((activeCell != cell.cell || activeRow != cell.row) && canCellBeActive(cell.row, cell.cell)) {
        if(!(e.ctrlKey || e.altKey || e.shiftKey || e.metaKey)){// pyk 2020 01 16
          if (!getEditorLock().isActive() || getEditorLock().commitCurrentEdit()) {
            scrollRowIntoView(cell.row, false);
            setActiveCellInternal(getCellNode(cell.row, cell.cell));
            if(! uu.getCurrFocusElem())setFocus();//pyk 2023 02 24 셀 편집 후 다른 셀 클릭시 포커스 없어져서 편집이 안되서
          }
        }
      }

      // pyk 2022 11 19 추가
      var item = self.getActiveItem();
      if(item){
        trigger(self.onItemClick, self.getActiveCellItem(), e);
      }
    }
    ...
    function setActiveCellInternal(newCell, opt_editMode) {
      ...
      if (activeCellNode != null) {
        ...
        activeItemId = self.getActiveItem().id;// pyk 2022 06 17
        ...
        if (opt_editMode == null) {
          opt_editMode = (activeRow == getDataLength()) || options.autoEdit || getColumns()[activeCell].autoEdit;
        }
        ...
        prevActiveRow = prevActiveItemId = activeItemId = null;//pyk 2022 06 17
      ...
      if (activeCellChanged) {
        ...
        //pyk 2022 06 16
        var rowChanged = prevActiveRow != activeRow;
        if(rowChanged){
          trigger(self.onRowChanged, getActiveCell());
          prevActiveRow = activeRow;
        }
        //end
      }

      var itemIdChanged = prevActiveItemId != activeItemId;
      if(itemIdChanged){
        trigger(self.onItemChanged, self.getActiveCellItem());
        prevActiveItemId = activeItemId;
      }
    }
    ...
    function isCellPotentiallyEditable(row, cell) {
      ...
      if(! self.isCellEditable(row, cell))return false;//pyk 2022 06 17
    ...
    function commitEditAndSetFocus() {
      ...
      if (getEditorLock().commitCurrentEdit()) {
        setFocus();
        if(true){// pyk 20200406 if (options.autoEdit) {
          //20210224 autocomplete 탭으로 값 선택시 포커스 문제로
          // pyk 2022/03/18 황요청
          // setTimeout(navigate, null, [options.navigateWhenEnter || 'down']);
        }
      }
    }
    ...
    $.extend(this, {
      ...
      onRowChanged: new Slick.Event(),// pyk 2022 06 16
      onItemChanged: new Slick.Event(),// pyk 2022 06 17
      onItemClick: new Slick.Event(),// pyk 2022 11 17
      ...
      applyColumnGroupHeaderWidths: applyColumnGroupHeaderWidths,
      // pyk 2015-10-29
      trigger: trigger,
      // pyk 2022-08-03
      uid: uid
