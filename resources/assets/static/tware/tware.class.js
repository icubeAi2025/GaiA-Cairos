        ...
        , async uploaderCreate(options) {
            ...
            class UploadFilesService {
                ...
                downloadFiles(fileMeta){
                    ...
                    link.href = `/resource/download?filePath=${encodeURIComponent(fileMeta.filePath)}&orgName=${encodeURIComponent(fileMeta.originalFilename)}`
                    link.download = fileMeta.originalFilename;
                    link.click();
                    link.remove();
                }
                ...
                if (document.querySelector('#vueTemplate')) {
                    ...
                    const uploaderComponent  = {
                        ...
                        methods: {
                            ...
                            isImage(ext) {
                                return ['jpg', 'gif', 'png'].includes(ext);
                            },
                            ...
                            getThumbnailStyle(info,mode){
                                ...
                                let style = {
                                    ...
                                    backgroundImage:'url(/assets/images/noimg.png)',
                                    backgroundSize: 'cover',
                                }
                                if(info.ext === 'txt'){
                                    style.backgroundImage = `url(/assets/images/ic_file_txt.png)`;
                                }
                                else if(info.ext === 'pdf'){
                                    style.backgroundImage = `url(/assets/images/ic_file_pdf.png)`;
                                }
                                else if(this.isImage(info.ext)){
                                    if(info.mode === "R"){
                                        let imagePath = this.makeImgPath(info.filePath);
                                        const img = new Image();
                                        img.onload = () => {
                                        ...
                                        style.backgroundImage = `url(${this.makeImgPath(info.src)})`;
                                    }
                                }
                                else{
                                    style.backgroundImage = 'url(/assets/images/ic_file_etc.png)'
                                }
                                return style;
                            },
                            openPreview(info){
                                ...
                                if(info.src){
                                    gaiaCommon.openPreview(pureFileMeta,info.src)
                                }
                                else{
                                    gaiaCommon.openPreview(pureFileMeta);
                                }
                            },
                            isValidUploadSize() {
                            ...
                            downloadFile(info){
                                ...
                                uploadFilesService.downloadFiles(pureFileMeta)
                            }
                        },
                        mounted()   {
